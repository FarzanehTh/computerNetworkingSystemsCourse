...
uint16_t cksum (const void *_data, int len) { /* len is the # of bytes in the packet */
  const uint8_t *data = _data;
  uint32_t sum;

  for (sum = 0;len >= 2; data += 2, len -= 2) /* working on 2 bytes at a time */
    sum += data[0] << 8 | data[1];
  if (len > 0)
    sum += data[0] << 8;
  while (sum > 0xffff)
    sum = (sum >> 16) + (sum & 0xffff);
  sum = htons (~sum); /* We convert the sum since the checksum in the packet is in the network order, so we have to do the comparison by that */
  return sum ? sum : 0xffff;  /* checksum value equal to 0xffff (i.e. the two least significant bytes are all 1s) shows the message is valid. */
}
