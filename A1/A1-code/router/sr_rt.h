...
struct sr_rt
{
    struct in_addr dest; /* Already in network order */
    struct in_addr gw;   /* Already in network order */
    struct in_addr mask; /* Already in network order */
    char   interface[sr_IFACE_NAMELEN]; /* name of the interface that maps the IP address dest to gw */
    struct sr_rt* next;
};
